...
parser = argparse.ArgumentParser(description='maximum acceptable error margin while matching strings')
parser.add_argument('-e', dest='error_margin', default=64, type=int,
                    help='maximum permitted difference between two strings that can still be considered to match')

parser.add_argument('-p', dest='processes', default=1, type=int,
                    help='Controls how many parallel processes are used when calculating the duplicates file.')

parser.add_argument('-f', dest='fastq_file', default=None, type=str,
                    help='address of the fastq file that will be used to create the dictionary')

parser.add_argument('-d', dest='duplicates_file', default=None, type=str,
                    help='if -f is used will write the duplicates file to this address. If not it will attempt to load it from this address')
...
#"""#argparse version
fastq_address = args.fastq_file
duplicates_address = args.duplicates_file
"""

#'''#Marcin version
fastq_address = os.path.join(os.getcwd(),"Dropbox","Marcin Python", "cancer", "CancerClassifier", "data",
"R2_100000.fastq")
duplicates_address = os.path.join(os.getcwd(),"Dropbox","Marcin Python", "cancer", "CancerClassifier", "Duplicates.csv")
PROCESSES=4
'''#MarcinB Version
#fastq_address = os.path.join(os.getcwd(), "data", "R2_100000.fastq")
#duplicates_address = os.path.join(os.getcwd(), "Duplicates.csv")
#PROCESSES=4
#'''
...
if(fastq_address != None):
    openings = getDictionary(fastq_address)
