...
duplicates_address = os.path.join(os.getcwd(), "Duplicates_31102018_CG.csv")
...
def getDictionary(fileName, tail):
    ...
    with open(fileName, 'r') as myFile:
        ...
        for line in myFile:
            i+=1
            if(i%4 == 2):
                start = line[:tail].strip()
                if("N" in start):
                    continue
                startSet.append(start)
                
    openingCombinations = dict.fromkeys(set(startSet))
    ...
    with open(fileName, 'r') as myFile:
        ...
        for line in myFile:
            ...
            if(i%4 == 2):
                ...
                if(openingCombinations[start] != None):
...
print("loaded the dictionary in ", time()-lsTime, "seconds")
